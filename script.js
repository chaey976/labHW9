...
base("bts").select({}).eachPage(gotPageOfPeople, gotAllPeople);
...
function showPeople() {
  ...
  people.forEach((person) => {
    ...
    /**TO DO: Write JavaScript code to add each character's information onto the webpage**/
      ...
      name.innerText = person.fields.name;
      peopleContainer.appendChild(name);

      var position = document.createElement("p");
      position.innerText = person.fields.position;
      peopleContainer.appendChild(position);
  });
}